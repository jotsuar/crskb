...
function gotDevices(mediaDevices) {
  if ($("#CustomerCrediventasForm").length) {return false;}
  select.innerHTML = '';
...
$(".fotoBtn").click(function(event) {
	event.preventDefault();
	InitSetPhoto($(this).data("canvas"),$(this).data("input"),$(this).data("img"), $(this).attr("id"));
});
...
$("#AssistAddForm").submit(function(event) {
    event.preventDefault();
});

$('#AssistAddForm').parsley().on('form:validate', function (formInstance) {
...
$("#AssistAddForm").submit(function(event) {
    ...
    if(!FORMULARIO_VALIDO){
        ...
        var formData            = new FormData($('#AssistAddForm')[0]);
        $.ajax({
            type: 'POST',
            ...
            success: function(result){
                $("#loaderKebco").hide();
                location.href = rootUrl+"assists/index";
            }
        });
      return true;
    }
    var document_file_up    = $("#AssistImageFile").val();

    if( !VIDEO_DATA || document_file_up == "" ){
        message_alert("La foto es requerida","error");
        event.preventDefault();
        return false;
    }
