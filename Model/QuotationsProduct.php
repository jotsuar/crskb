...
class QuotationsProduct extends AppModel {
	...
	public function get_data_quotation($quotation_id, $todo = null){
		...
		$order  			= array("QuotationsProduct.id" => "ASC" );
	...
	public function beforeSave($options = array()) {
		...
		if (!isset($this->data["QuotationsProduct"]["id"])) {
			$this->data["QuotationsProduct"]["purchase_price_usd"] = $this->Product->field("purchase_price_usd",["id"=>$this->data["QuotationsProduct"]["product_id"]]);
			$this->data["QuotationsProduct"]["purchase_price_cop"] = $this->Product->field("purchase_price_cop",["id"=>$this->data["QuotationsProduct"]["product_id"]]);
			$this->data["QuotationsProduct"]["purchase_price_wo"] = $this->Product->field("purchase_price_wo",["id"=>$this->data["QuotationsProduct"]["product_id"]]);
		}

		return true;
	}

	public function get_data_quotation_for_importer_2($quotation_id, $products_ids = []){
		
		$this->recursive 	= 0;
		$fields				= array('QuotationsProduct.quantity','QuotationsProduct.product_id');
		$conditions			= array('QuotationsProduct.quotation_id' => $quotation_id);
		if (!empty($products_ids)) {
			$conditions["QuotationsProduct.product_id !="] = array_values($products_ids);
		}
		return $this->find('list',compact('conditions','fields'));
	}

	public function get_data_quotation_for_importer($quotation_id, $products_ids = []){
	...
	public function total_data_quotation_products($cotizacion_id){
		$this->recursive 	= -1;
		$conditions			= array('QuotationsProduct.quotation_id' => $cotizacion_id);
		return $this->field('SUM( QuotationsProduct.quantity ) as total', $conditions);
	}
	...
	public function find_quotation_id($product_id, $grupo = null, $type = null, $ini = null, $end = null){
		$this->recursive 	= -1;
		...
		$order 				= ["QuotationsProduct.created" => "DESC"];
		$limit 				= !is_null($type) && $type == "venta" ? 10000 : 1000000;
		...
		if(!is_null($grupo)){
			...
			if (!is_null($ini) || !is_null($end) ) {
				$conditions["DATE(QuotationsProduct.created) >="] = $ini;
				$conditions["DATE(QuotationsProduct.created) <="] = $end;
			}

			$datos  = $this->find('all',compact('conditions','fields','limit','order'));
		...
		return $this->find('list',compact('conditions','fields','limit','order'));
	}
