			...
			<?php if (!empty($otherProducts )): ?>								
				<?php foreach ($otherProducts as $key => $product): ?>
					<tr>
						...
						<td class="size11">
							<?php echo $this->element("products_block",["producto" => $product["Product"],"inventario_wo" => $partsData[$product["Product"]["part_number"]] ]) ?>
						</td>
						...
						<td>
							...
							<a href="" class="btn btn-success selectOtherProduct" data-id="<?php echo $product['Product']['id'] ?>" data-request="<?php echo $requestId ?>" data-import="<?php echo $importId ?>" data-currency="<?php echo $currency ?>">
