...
<script>
	var productsTable 	= {};
	var trmActual 		= <?php $trmActual = is_null($trmActual) || $trmActual == false ? 3900 : $trmActual ; echo $trmActual; ?>;
...
<div class="col-md-12 col-lg-12 p-0">
	...
	<div class="bockwhite-import spacebtn20 blockwhite">		
		<div class="row">
			<h2 class="titlemenuline">Inventario de alta rotación</h2>
		</div>
	</div>
			...
			<?php foreach ($requests as $key => $request): ?>
				<li class="nav-item">
					...
					<?php if (!is_null($internacional)): ?>
						<a class="nav-link <?php echo $clase ?> brandsLink" id="<?php echo md5($request["Brand"]["id"]) ?>-tab" data-toggle="tab" href="#<?php echo md5($request["Brand"]["id"]) ?>" role="tab" aria-controls="<?php echo md5($request["Brand"]["id"]) ?>" aria-selected="true" data-brand="<?php echo md5($request["Brand"]["id"]) ?>" data-url="<?php echo $this->Html->url(array("controller"=>"products","action" => "new_panel","internacional","?" => array("brand_id" => $request["Brand"]["id"]))) ?>">
		...
		<div class="tab-content" id="myTabContent">
			...
			<?php foreach ($requests as $key => $request): ?>
				...
				<div class="tab-pane fade <?php echo $clase ?>" id="<?php echo md5($request["Brand"]["id"]) ?>" role="tabpanel" aria-labelledby="<?php echo md5($request["Brand"]["id"]) ?>-tab" >
					<div class=" blockwhiteabajo2 spacebtn20">
								...
								<div class="row">
									<div class="col-md-12 mb-3">
										<?php if (AuthComponent::user("role") == "Gerente General"): ?>
											...
											<input type="checkbox" class="checkAllProducts check_all_<?php echo md5($request["Brand"]["id"]) ?>" value="<?php echo $request["Brand"]["id"] ?>" data-id="<?php echo md5($request["Brand"]["id"]) ?>" data-class="check_prod_<?php echo md5($request["Brand"]["id"]) ?>" data-delete="delete_<?php echo md5($request["Brand"]["id"]) ?>">
										...
										<?php endif ?>
										<a href="javascript:void(0)" data-class="check_prod_<?php echo md5($request["Brand"]["id"]) ?>" class="btn btn-info delete_<?php echo md5($request["Brand"]["id"]) ?> delete_requests" style="display: none;">
								    ...
								    <div class="card-header reset <?php echo Configure::read("TYPE_REQUEST_IMPORT_DATA.".$keyType) ?>" id="hheading<?php echo md5($keyType) ?>_<?php echo md5($request["Brand"]["id"]) ?> ">
								      <h5 class="mb-0">
								        <button class="btn btn-link bold " data-toggle="collapse" data-target="#collapse<?php echo md5($keyType) ?>_<?php echo md5($request["Brand"]["id"]) ?>" aria-expanded="true" class="text-white" aria-controls="collapse<?php echo md5($keyType) ?>_<?php echo md5($request["Brand"]["id"]) ?>">
								          <?php echo Configure::read("TYPE_REQUEST_IMPORT_DATA.".$keyType) ?> 
								        </button>
								      </h5>
								    </div>
											    ...
											    <div class="card-header reset <?php echo Configure::read("TYPE_REQUEST_IMPORT_DATA.".$details["type_request"]) ?> " id="headingOne">
												    ...
												    <h2 class="mb-0 displayinline">
												        <button class="btn btn-link" type="button" data-toggle="collapse" data-target="#collapseOne" aria-expanded="true" aria-controls="collapseOne">
												          ...
												          <small> Solicita <b><?php echo $details["InfoProducts"]["User"]["name"] ?></b> el <?php echo $this->Utilities->date_castellano(h($details['created'])) ?></small>
												        </button>
												    </h2>
												    <?php if ($details["type_request"] == Configure::read("TYPE_REQUEST_IMPORT.SALES_NO_INVENTORY")): ?>
												        <?php if(!empty($details["prospective_user_id"])): ?>									<span class="pull-right">
																<a href="#" class="classInformeCliente btn btn-outline-danger mt-1" data-uid="<?php echo $details["id"] ?>">
													...
													<span class="pull-right savebefore mr-3 ">
														<?php if ($details["type_request"] == 3): ?>
															<a href="#" class="btn btn-secondary btnReload" data-id="<?php echo $details["id"] ?>">
																Devolver a reposición de inventario
																<i class="fa fa-retweet vtc"></i>
															</a>
														<?php else: ?>

															<input type="checkbox" class="mt-2 wishList wish_check_prod_<?php echo md5($request["Brand"]["id"]) ?>" data-wish="<?php echo md5($details["id"]) ?>" data-check="detail_<?php echo md5($details["id"]) ?>" data-class="check_prod_<?php echo md5($request["Brand"]["id"]) ?>" value="<?php echo $details["id"] ?>" data-brand="<?php echo md5($request["Brand"]["id"]) ?>" > 
											    ...
											    <div id="collapseOne" class="collapse show" aria-labelledby="headingOne" data-parent="#accordionExample">
												    <div class="card-body">
												    			...
												    			<?php if ($details["type_request"] == Configure::read("TYPE_REQUEST_IMPORT.SALES_NO_INVENTORY")): ?>
													    			<li>
													    				<b>Flujo de negocio</b>: 
																			...
																			<?php if ($relacionados): ?>
																				<a href="#" class="viewRelations btn btn-outline-success" data-id="<?php echo $details["prospective_user_id"] ?>" data-request="<?php echo $details["import_request_id"] ?>" >
														...
														<table class="table-striped table-bordered myTable">
															<thead>
																<tr>
																	<th class="text-center">Añadir al pedido</th>
																	<th>Imagen</th>
																	<th>Producto</th>
																	...
																	<th class="size1">Inventario actual</th>							
																	<th class="size1">Configuración</th>							
																	<th class="size1">Cant. a importar</th>
																	<th class="size1">
																		Costo 
																	...
																	<tr class="os1">
																		<td class="text-center size1">
																			...
																			<input 
																				...
																				data-delete="delete_<?php echo md5($request["Brand"]["id"]) ?>"
																		...
																		<td>
																			...
																			<img dataimg="<?php echo $this->Html->url('/img/products/'.$ruta) ?>" dataname="<?php echo h($value['name']); ?>" src="<?php echo $this->Html->url('/img/products/'.$ruta) ?>" width="65px" height="65px" class="imgmin-product mb-3">
																		...
																		<?php if ($details["type_request"] == Configure::read("TYPE_REQUEST_IMPORT.SALES_NO_INVENTORY")): ?>
																			<td>
																				...
																				<?php else: ?>
																					<span class="bg-warning">¡Se debe entregar en  <?php echo abs($dataDay) ?> día(s)! - <?php echo date("Y-m-d", strtotime("+".$dataDay." day")) ?></span>
																				<?php endif ?>
																		...
																		<td class="controlquantity">
																			<?php echo $this->element("products_block",["producto" => $value, "inventario_wo" => $inventioWo[$value['part_number']], "bloqueo" => false, "reserva" => isset($inventioWo["Reserva"][$value["part_number"]]) ? $inventioWo["Reserva"][$value["part_number"]] : null]) ?>
																		</td>
																		<td>
																			
																			Inv. min: <?php echo $value["min_stock"]; ?> <br>
																			Inv. max: <?php echo $value["max_cost"]; ?> <br>
																			Inv. rer: <?php echo $value["reorder"]; ?> 
																		...
																		<td> 
																			...
																			<?php $margenFinal = $this->Utilities->calculateMargen($trmActual,$this->Utilities->getProductFactor($value["id"]),$value["purchase_price_usd"],$datosProductoVenta,$value["ImportRequestsDetailsProduct"]["quantity"], "usd"); 
																				?> 
																			<span class="viewdetailc btn btn-outline-primary" style="padding: 0px !important;">
																					...
																					<?php if ($margenFinal < 30): ?>
																						<a class="pointer bg-danger p-1 text-white aa" data-id="calculate_<?php echo md5($value["id"]) ?>_<?php echo md5($value["id"]) ?>">
																			...
																			<a href="#" data-id="<?php echo $value["id"] ?>" class="btn btn-outline-primary notesProduct " data-toggle="tooltip" title="" data-original-title="Gestionar notas del producto">
																				<i class="fa fa-comments vtc"></i>
																			</a>
																		</td>
																	</tr>
																	...
																	<?php if ($details["type_request"] == Configure::read("TYPE_REQUEST_IMPORT.SALES_NO_INVENTORY")): ?>													
																		<tr class="os2">
																			<td colspan="12" id="calculate_<?php echo md5($details["id"]) ?>_<?php echo md5($value["id"]) ?>" class="removemg usd_brands_<?php echo md5($request["Brand"]["id"]) ?>">
																				<?php $product_id = $value["id"]; $currency = "usd"; $costoProducto = $value["purchase_price_usd"]; $productoCantidad = $cantidadProducto = $value["ImportRequestsDetailsProduct"]["quantity"];  ?>
																				<?php echo $this->element("trm_import", compact("costoProducto","datosProductoVenta","currency","product_id","margenFinal","cantidadProducto")); ?>		
																	...
																	<?php else: ?>
																		<tr class="os2">
																			<td colspan="12" id="calculate_<?php echo md5($details["id"]) ?>_<?php echo md5($value["id"]) ?>" class="removemg usd_brands_<?php echo md5($request["Brand"]["id"]) ?>">
																				...
																				<?php echo $this->element("trm_import", compact("costoProducto","datosProductoVenta","currency","product_id","margenFinal","productoCantidad","cantidadProducto")); ?>
																			</td>
																		</tr>
					...
					<div class=" blockwhite spacebtn20">
						<div class="row">
							...
							<div class="col-md-6 mt-4">
								<div class="input textarea border pl-1 pr-1 pt-3 pb-1 border-info">
								...
								<div class="row">
									...
									<div class="col-md-6">
										<div class="form-group mt-4">
											<label for="formaPago">Por favor selecciones la forma de pago al proveedor</label>
