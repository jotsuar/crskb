	...
	<div class=" blockwhite spacebtn20">
		<div class="row ">
			<div class="col-md-12 text-center">
				<h1 class="nameview">HISTORIAL DE TODOS LOS PAGOS VERIFICADOS POR DATÁFONO</h1>
			</div>
		</div>
	</div>
			...
			<table cellpadding="0" cellspacing="0" class='table-striped datosPendientesDespacho table-bordered'>
				<thead>
					<tr>
						...
						<th style="width: 300px;">Disminuir comisión</th>
						<th>Medio de pago / Identificador</th>
						<th>Imagen</th>
						<th>Tipo de pago</th>
					</tr>
				</thead>
				<tbody>
					<?php foreach ($paymentTrue as $value): ?>
						<tr>
							...
							<td>
								<?php if (!is_null($value["ClientsLegal"]["name"])): ?>
							...
							<td>$<?php echo number_format((int)h($value['ProspectiveUser']['valor']*1.19),0,",","."); ?>&nbsp;</td>
							<td>
								<div class="pointer price-purchase_price_usd cambioCostoDataUsd" data-id="<?php echo $value["ProspectiveUser"]["id"] ?>" data-price="<?php echo $value['ProspectiveUser']['value_minus'] ?>">
										<?php echo $value['ProspectiveUser']['value_minus']; ?> %&nbsp;
								</div>
							...
							<?php } else { ?>
								<td class="comprobanteimgTd">
									<a datacomprobantet="<?php echo $this->Html->url('/img/flujo/pagado/'.$value['FlowStage']['document']) ?>" src="<?php echo $this->Html->url('/img/flujo/pagado/'.$value['FlowStage']['document']) ?>" class="reciboT btn" width="30px">Ver imagen</a> 
								</td>
							<?php } ?>
...
<script>
	var RECIBO_HISTORY = 1;
</script>

<style>
	#formularioQueCambia > button,#formularioQueCambia > input{
		width:  100% !important;
		margin-top: 10px;
	}
	#formularioQueCambia{
