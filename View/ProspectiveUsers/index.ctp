				...
				<div class="row">			
					<nav class="navbar navbar-expand-lg navbar-light bg-filter p-1">
						<div class="collapse navbar-collapse" id="navbarTogglerDemo01">
							<b><a href="<?php echo $this->Html->url(array('controller'=>'ProspectiveUsers','action'=>'index')) ?>">Todos</a></b>
						...
						) == "Gerente General" || AuthComponent::user("role") == "Servicio al Cliente" || AuthComponent::user("role") == "Contabilidad"): ?>
								<a href="<?php echo $this->Html->url(array('controller'=>'ProspectiveUsers','action'=>'index_externals')) ?>">Flujos Externos</a>
								<a href="<?php echo $this->Html->url(array('controller'=>'ProspectiveUsers','action'=>'index_remarketing')) ?>">Flujos remarketing</a>
							<?php endif ?>
						</div>
						...
						<div class="dropdown text-right mr-2">
							...
							<div class="dropdown-menu datastates2">
								<?php
									...
									$label 						= 'Pagado (todos)'.$count_pagado;
									...
									$classAdd 					= !empty($etapa) && $etapa == 57 ? "active" : "";
									$label 						= 'Pagado con importación '.$count_withImport;
									$urlParams['filterEtapa'] 	= 57;
									echo $this->Html->link($label, array('?' => $urlParams),array('class' => 'dropdown-item '.$classAdd));

									$classAdd 					= !empty($etapa) && $etapa == 58 ? "active" : "";
									$label 						= 'Pagado sin despacho '.$count_without;
									$urlParams['filterEtapa'] 	= 58;
									echo $this->Html->link($label, array('?' => $urlParams),array('class' => 'dropdown-item '.$classAdd));

									$classAdd 					= !empty($etapa) && $etapa == 59 ? "active" : "";
									$label 						= 'Pagado despacho parcial '.$count_parcial;
									$urlParams['filterEtapa'] 	= 59;
									echo $this->Html->link($label, array('?' => $urlParams),array('class' => 'dropdown-item '.$classAdd));
						...
						<div class="dropdown text-right">
							...
							<div class="dropdown-menu datastates2">
								...
								<?php foreach ($usuariosAsesores as $valueUsuarios){
									...
									$classAdd 					= !empty($usuario) && $usuario == $valueUsuarios['User']['id'] ? "active" : "";
									echo $this->Html->link($valueUsuarios['User']['name'], array('?' => $urlParams ),array('class' => 'dropdown-item '.$classAdd));
								} ?>
							</div>
						</div>
				...
				<div class="col-md-12">
					<div class="input-group stylish-input-group mb-2">
						<?php if (isset($this->request->query['q'])){ ?>
		                    ...
		                    <script>
		                    	const FLUJO_ACTUAL = '<?php echo $this->request->query['q']; ?>';
		                    </script>
						<?php } else { ?>
			...
			<div class="prospectiveUsers index row">
				<div class="col-md-12">
					...
					<?php foreach ($prospectiveUsers as $prospectiveUser): ?>
									...
									<h3 class="opendetails" data-uid="<?php echo $prospectiveUser['ProspectiveUser']['id'] ?>" data-type="<?php echo $prospectiveUser['ProspectiveUser']['contacs_users_id'] ?>">
										...
										<span class="razonflow">
											...
											<?php if ($prospectiveUser["ProspectiveUser"]["state_flow"] == 1 && is_null($prospectiveUser["ProspectiveUser"]["clients_natural_id"]) && $prospectiveUser["ProspectiveUser"]["contacs_users_id"] == 0 ): ?>
												...
												<?php if ( (AuthComponent::user("id") == $prospectiveUser["ProspectiveUser"]["user_id"] || AuthComponent::user("role") == "Gerente General" )  && $prospectiveUser["ProspectiveUser"]["reject"] == 0 ): ?>
						...
						<?php endif; ?>
						<?php if ($prospectiveUser["ProspectiveUser"]["remarketing"] == 1 ): ?>
							<span class="internacional-tag bg-blue mr-2">REMARKETING </span>							
						<?php endif; ?>
							<?php if ($prospectiveUser["User"]["role"] == "Asesor Externo"): ?>
								<span class="internacional-tag" style="background: red;"><small>Asesor externo</small></span>
							<?php endif ?>
									...
									<?php endif ?>
									<?php if( AuthComponent::user('role') == Configure::read('variables.roles_usuarios.Logística') || AuthComponent::user('role') == Configure::read('variables.roles_usuarios.Gerente General') || AuthComponent::user("email") == 'ventas2@almacendelpintor.com' ): ?>
										<div class="linealign">
							...
							<div class="row bs-wizard" >
								...
								<?php if (!is_null($prospectiveUser["ProspectiveUser"]["clients_natural_id"]) || $prospectiveUser["ProspectiveUser"]["contacs_users_id"] > 0): ?>
									...
									<div class="col-md-2 bs-wizard-step <?php echo $this->Utilities->validate_state_negociado($prospectiveUser['ProspectiveUser']['id'],$prospectiveUser['ProspectiveUser']['state_flow']); ?>">
										<div class="progress"><div class="progress-bar"></div></div>
										<span class="bs-wizard-dot state_negociado" data-valid="<?php echo $prospectiveUser["ProspectiveUser"]["valid"] ?>" data-uid="<?php echo $prospectiveUser["ProspectiveUser"]["valid"] == 1 ? 0 : $prospectiveUser['ProspectiveUser']['id'] ?>" data-id="<?php echo $prospectiveUser["ProspectiveUser"]["valid"] == 1 ? 0 : $this->Utilities->encryptString($prospectiveUser['ProspectiveUser']['id']) ?>" data-state="<?php echo $prospectiveUser['ProspectiveUser']['state_flow'] ?>"></span>
									...
									<div class="col-md-2 bs-wizard-step <?php echo $this->Utilities->validate_state_pagado($prospectiveUser['ProspectiveUser']['id'],$prospectiveUser['ProspectiveUser']['state_flow']); ?>">
										<div class="progress"><div class="progress-bar"></div></div>
										<span class="bs-wizard-dot state_pagado" data-valid="<?php echo $prospectiveUser["ProspectiveUser"]["valid"] ?>" data-uid="<?php echo $prospectiveUser["ProspectiveUser"]["valid"] == 1 ? 0 : $prospectiveUser['ProspectiveUser']['id'] ?>" data-state="<?php echo $prospectiveUser['ProspectiveUser']['state_flow'] ?>"></span>
							...
							<div class="row">
								<div class="col-lg-12 allnovedad_<?php echo $prospectiveUser['ProspectiveUser']['id'] ?> <?php echo is_null($prospectiveUser["ProspectiveUser"]["clients_natural_id"]) &&  $prospectiveUser["ProspectiveUser"]["contacs_users_id"] == 0 ? "allnovedad" : "bg-white" ?> dnone">
									<div class="row">
										<div class="col-md-8">
...
<div class="popup" style="width: 60%; max-height: 700px; overflow-y: auto;">
...
<?php
	...
	echo $this->Html->script("controller/prospectiveUsers/index.js",			array('block' => 'AppScript')); 
?>
...
<?php echo $this->Html->script("controller/prospectiveUsers/despacho.js",			array('block' => 'AppScript'));  ?>
<?php echo $this->Html->script("controller/prospectiveUsers/recibos.js",			array('block' => 'AppScript'));  ?>
<?php echo $this->Html->script("controller/prospectiveUsers/especial_customer.js",			array('block' => 'AppScript'));  ?>

<?php echo $this->element("picker"); ?>
 <?php echo $this->Html->script("controller/inventories/detail.js",    array('block' => 'AppScript')); ?>
