...
<?php echo $this->Form->create('ProspectiveUser',array("id" => "formGeneralCustomer","type" => "file","url"=>["controller" => "prospective_users","action" =>"add_prospective"])); ?>
      <div class="row"> 
				...
				<div class="row">
					<div class="col-md-9 col-lg-9">
						<?php
							...
							$options = [];
							if (!empty($cliente)) {
								if (isset($cliente["ClientsNatural"])) {
									$default = $cliente["ClientsNatural"]["id"]."_NATURAL";
									$options = [$default => $cliente["ClientsNatural"]["identification"]." | ".$cliente["ClientsNatural"]["name"]." | ".$cliente["ClientsNatural"]["email"] ]; 
								}else{
									$default = $cliente["ClientsLegal"]["id"]."_LEGAL";
									$options = [$default => $cliente["ClientsLegal"]["nit"]." | ".$cliente["ClientsLegal"]["name"]]; 
								}
							}
						?>
						<?php echo $this->Form->input('clients_natural_id',array('label' => 'Selecionar Cliente',"required" , "type" => "select" , "options" => $options, "default" => $default , "empty" => "Busca y selecciona un cliente" )); ?>
					</div>
					<?php if (empty($cliente)): ?>
						<div class="col-md-3 col-lg-3 text-center ">
							<div class="btn btn-success alignforlabel">	
								<i class="fa fa-x fa-plus-circle addNewCustomerProspectiveWo" data-type="1"> <span>Crear nuevo cliente</span></i> 
							</div>
						</div>
					<?php endif ?>
				</div>
        ...
        <div class="col-md-10">
          <?php echo $this->Form->input('origin',array('label' => 'Origen de la Solicitud:', 'options' => ["Remarketing" => "Remarketing" ], "required" )); ?>
        </div>
      ...
      <div id="user_asignado_div" class="mt-2">
        <?php if (AuthComponent::user("role") != "Asesor Externo"): ?>
          <?php echo $this->Form->input('user_id',array('label' => 'Asignado al asesor:','value'=>AuthComponent::user('id'), 'options' => (isset($usuarios)) ? $usuarios: "", "empty" => "Seleccionar asesor","required"));  ?>
        <?php else: ?>
